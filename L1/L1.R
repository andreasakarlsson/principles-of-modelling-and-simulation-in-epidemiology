...
#  DESCRIPTION
#  ===========
#  This is a extended version of the script compile_minimalistic_analysis_dataframe.R -- 
#  In this extended version the variables used by create_exploratory_figs.R are compiled 
#  as well as other variables that might be used in future development of EDA figs.  
...
## Plot results
##-----------------------------
matplot(times, out, type = 'l', xlab = 'Time', ylab = 'AU', main = 'Dispersion of a flow that passes a dynamic structure', lty = 1, bty = 'l', col = 2:3)
legend('topright', c('5*t','-5*t'), lty = 1, col = 2:3)
...
## Plot results
##-----------------------------
matplot(times, out, type = 'l', xlab = 'Time', ylab = 'AU', main = 'Positive Feedback', lty = 1, bty = 'l', col = 2:4)
...
## Plot results
##-----------------------------
matplot(times, out, type = 'l', xlab = 'Time', ylab = 'AU', main = 'Negative Feedback', lty = 1, bty = 'l', col = 2:4)
legend('topright', c('T=5','T=10','T=20'), lty = 1, col = 2:4)
...
## Plot results
##-----------------------------
matplot(times, out, type = 'l', xlab = 'Time', ylab = 'AU', main = 'Avarage sojourn time', lty = 1, bty = 'l', col = 2:3)
...
matplot(times, out, type = "l", xlab = "Time", ylab = "AU", main = "Negative feedback over two compartments", lwd = 1, lty = 1, bty = "l", col = 2:3)
...
## Plot results
##-----------------------------
matplot(times, out, type = c('o','l'), xlab = 'Time', ylab = 'AU', main = 'Logistic Model & SI-model', pch = 1, lwd =4, col = 2:4)
legend('topright', c('Logistic Model','SI-model'), pch = c('o','-'), lwd =c(0,4), col = 2:4)
...
times <- seq(0, 100, by = 0.01)
out <- as.data.frame(ode(y = init, times = times, func = deriv, parms = parameters, method='euler'))
out$time <- NULL

##-----------------------------
## Plot results
##-----------------------------
matplot(times, out, type = 'l', xlab = 'Time', ylab = 'AU', main = 'Logistic Model & SI-model', pch = 1, col = 2)
...
for (tt in 1:length(d_t)){
  ...
  out$time <- NULL
  if (tt==1)
    matplot(times, out, type = 'l', xlab = 'Time', ylab = 'AU', main = 'Choosing time-step', pch = 1, col = 1)
  else
...
signal <- as.data.frame(list(times = times, import = rep(0,length(times))))
signal$import[signal$times >= 0 & signal$times <= 0] <- 1 / d_t * 2 #The 2 since it is a triangle
sigimp <- approxfun(signal$times, signal$import, rule = 2)
...
## Plot results
##-----------------------------
matplot(times, out[c('X1','X2_2','X3_3')], type = 'l', xlab = 'Time', ylab = 'AU', main = 'Multiple compartments', lwd = 1, lty = 1, bty = 'l', col = 2:5, ylim=c(0,0.1))
legend('topright', c('1 Compartment', '2 Compartments','3 Compartments'), lty = 1, col = 2:5)
