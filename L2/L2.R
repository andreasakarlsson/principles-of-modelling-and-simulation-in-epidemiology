...
nu <- matrix(c(+1,-1),ncol=2)
for (i in c(1:10)){
  out <- ssa(x0, a , nu, parms, tf=100, tau = 1, method="ETL", maxWallTime=5)
  #'Explicit tau-leap' => user-defined step size
...
median_quantiles <- quantile(within_summary$X_end, c(.025, .50, .975))
...
ggplot(data = within_summary, aes(x=Repetition, y=X_end, colour = Percentile)) +
  ...
  geom_hline(data=mean_sd,aes(yintercept=mean-1.9604*sd,3),linetype=2, colour="#990000") +
  geom_point(aes(group = Percentile)) + ggtitle('X(100), Mean & 95% CI') + ylab("X(100)")
...
for (i in 1:10){
  ...
  #'Explicit tau-leap' => user-defined step size
  #ignoreNegativeState => gives warning instead of error for negative number of susceptibles.
  Repetition <- rep(i,nrow(out$data))
  Sample <- 1:nrow(out$data)
...
ggplot(data = within_summary, aes(x=Repetition, y=X_end))  + geom_point() +
  ...
  geom_hline(data=mean_sd,aes(yintercept=mean+1.9604*sd,3),linetype=2, colour="#990000") +
  geom_hline(data=mean_sd,aes(yintercept=mean-1.9604*sd,3),linetype=2, colour="#990000") +
  facet_wrap( ~ Model, ncol=2) + ggtitle('X(20), Mean & 95% CI') + ylab("X(20)")
